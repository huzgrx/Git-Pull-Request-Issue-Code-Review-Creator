...
const { PullMergeBot } = require("./index");
require("dotenv").config();
const program = new Command();
program
  .name("pull-merge-bot")
  .description(
    "🤖 Automated GitHub bot for creating PRs, code reviews, and issues"
  )
  .version("2.0.0");
program
  .command("run")
  .description("Run the Pull-Merge Bot with configuration")
  .option("-t, --token-a <token>", "GitHub token for Account A (invited user)")
  .option(
    "-T, --token-b <token>",
    "GitHub token for Account B (repository owner)"
  )
  .option("-o, --repo-owner <owner>", "Repository owner username")
  .option("-r, --repo-name <name>", "Repository name")
  .option("-f, --fork-owner <owner>", "Fork owner username (Account A)")
  .option("-b, --branch-name <branch>", "Branch name (default: jonny)")
  .option("-i, --create-issue", "Enable issue creation")
  .option("-I, --issue-type <type>", "Issue type: bug, feature, or enhancement")
  .option("-t, --issue-title <title>", "Custom issue title")
  .option("-B, --issue-body <body>", "Custom issue body")
  .option("-c, --enable-code-review", "Enable code review")
  .option(
    "-p, --random-chance <percentage>",
    "Random chance percentage (1-100, default: 30)"
  )
  ...
  .action(async (options) => {
    try {
      ...
      const config = {
        ...
        enableCodeReview:
          options.enableCodeReview || process.env.ENABLE_CODE_REVIEW === "true",
        randomChancePercentage:
          parseInt(
            options.randomChance || process.env.RANDOM_CHANCE_PERCENTAGE
          ) || 30,
      };
      ...
      if (missingFields.length > 0) {
        ...
        console.error(
          chalk.blue(
            "   - Example: pull-merge-bot run --token-a <token> --repo-owner <owner>"
          )
        );
        process.exit(1);
      }
      ...
      if (
        config.randomChancePercentage < 1 ||
        config.randomChancePercentage > 100
      ) {
        console.error(
          chalk.red("❌ Random chance percentage must be between 1 and 100")
        );
        process.exit(1);
      }
      ...
      console.error(
        chalk.red("❌ Error running Pull-Merge Bot:"),
        error.message
      );
      process.exit(1);
    }
...
program
  ...
  .action(() => {
    ...
    const config = {
      TOKEN_A: process.env.TOKEN_A
        ? "***" + process.env.TOKEN_A.slice(-4)
        : "Not set",
      TOKEN_B: process.env.TOKEN_B
        ? "***" + process.env.TOKEN_B.slice(-4)
        : "Not set",
...
program
  ...
  .action(async () => {
    ...
    console.log(
      chalk.yellow(
        "This wizard will help you create a .env file with your configuration.\n"
      )
    );
