...
class PullMergeBot {
  constructor(config) {
    ...
    this.octokitA = new Octokit({ auth: this.config.tokenA });
    this.octokitB = new Octokit({ auth: this.config.tokenB });
  }
  ...
  async updateBranchFromMain() {
    try {
      ...
      console.error(
        `❌ Error updating ${this.config.branchName} branch:`,
        error.message
      );
      return false;
    }
  ...
  async createOrUpdateBranch() {
    try {
      ...
      try {
        ...
        if (error.status === 404) {
          console.log(
            `📝 Creating ${this.config.branchName} branch from main...`
          );
          ...
          console.log(
            `✅ ${this.config.branchName} branch created successfully`
          );
          return true;
        }
        throw error;
      ...
      console.error(
        `❌ Error creating ${this.config.branchName} branch:`,
        error.message
      );
      return false;
    }
...
- Bot version: 2.0.0`;
  ...
  async addCodeReviewComment(prNumber, comment) {
    try {
      ...
      console.log(
        `✅ Code review comment added to PR #${prNumber} by ${this.config.forkOwner}`
      );
      return true;
    } catch (error) {
  ...
  async performCodeReview(prNumber) {
    try {
      console.log(
        `🔍 Performing code review on PR #${prNumber} by ${this.config.forkOwner}...`
      );
      ...
      const overallComment = `## 🤖 Automated Code Review by ${
        this.config.forkOwner
      }
...
---
*This review was performed automatically by the Pull-Merge Bot for ${
        this.config.forkOwner
      }*`;
      ...
      console.log(
        `✅ Code review completed by ${this.config.forkOwner} and approved by ${this.config.repoOwner}`
      );
      return true;
    } catch (error) {
...
*This PR will be automatically reviewed and merged by the bot.*`;
      ...
      const pr = await this.octokitA.pulls.create({
        ...
        head: this.config.branchName,
        base: "main",
        title: `🤖 Automated PR: Update README.md from ${
  ...
  async run() {
    try {
      ...
      console.log(
        `🔍 Code Review: ${this.config.enableCodeReview ? "Yes" : "No"}`
      );
      ...
      if (existingPR) {
        ...
        console.log(
          `✅ Existing PR #${existingPR.number} merged successfully.`
        );
        return true;
      }
